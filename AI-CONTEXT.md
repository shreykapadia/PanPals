...
| Path                                                                                                                                                                                                                                                                                                            | Owner          | Area                                                                                                                                                                                                                                                                        |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app/_layout.tsx`, `app/index.tsx`, `app/(tabs)/_layout.tsx`, `app/(auth)/*`, `app/(tabs)/you.tsx`, `supabase/*`, `types/database.ts`, `lib/*`, `theme/*`, `mocks/*`, `components/ui/*`, `components/onboarding/*`, `components/you/*`, `features/auth/*`, `features/you/*`, `docs/*`, `.github/*`, `scripts/*` | **Shrey** ONLY | **Platform + entire backend**: scaffold, auth, navigation, Supabase schema/RLS/RPCs/seed, generated types, shared UI kit, `lib/api` data hooks, catalog search service, analytics, onboarding + profile/You, cross-cutting empty/error patterns. Reviews & merges every PR. |
...
- **`features/*` is not automatically a feature lane.** Four of its folders belong
  to the four owners (`home`, `wishlist`, `inventory`, `empties`); `features/auth/*`
  and `features/you/*` are platform copy for Shrey's own screens and are his. The
  same goes for `components/*`: `ProgressRing.tsx` is Aaron's, everything else
  (`ui/`, `onboarding/`, `you/`) is Shrey's. Match the path, not the parent folder.

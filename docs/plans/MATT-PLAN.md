...
- **`useUpdateProduct()`** — unblocks edit; your `editAction`/`editTitle`/`saveEdit` strings finally get UI. Note it **throws** on `status: 'finished'`; finishing stays Talbia's flow.

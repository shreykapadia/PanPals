...
3. Set visibility to **Public**. _Why: required-review branch protection is free
...
"MCP" is the bridge that lets your AI tools _directly_ read your Supabase database
...
`PanPals MCP`. Copy it now (you can't see it again). _This is different from the
anon/service keys — it's what the MCP server authenticates with._
...
approve in the browser. _(Prefer a token instead? Add
...
Then in a Claude Code chat, ask: _"Using the Supabase MCP, list my tables"_ and
_"Using the GitHub MCP, list open pull requests on panpals."_ Both should return
real data.
...
Paste this, filling in your project ref and token(s). _(Note: Antigravity uses
`serverUrl` for remote servers, and `command`/`args`/`env` for local ones.)_
...
In an Antigravity chat with Gemini, ask the same two checks: _"list my Supabase
tables"_ and _"list open PRs on panpals."_ Both tools now manage the same repo and
database.
